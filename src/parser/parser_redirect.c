	...
	t_token *token, enum e_type type)
{
	...
	if (res[i].redirect_file == NULL && free_group_redirect(res, num_redirect + 1)
...
int	parse_redirect(t_list **lexer, t_group *group)
{
	...
	{
		group->reverse_redirect = add_redirect(group->reverse_redirect, (group->number_reverse_redirect)++, ((t_token *) ((*lexer)->next->content)),

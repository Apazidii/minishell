...
/*   Updated: 2022/08/24 17:40:29 by dgalactu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "minishell.h"
#include "parser.h"

t_redirect	*add_redirect(t_redirect *redirect, int num_redirect,
	t_token *token, enum e_type type)
{
	t_redirect	*res;
	int			i;
	...
	if (res[i].redirect_file == NULL && \
	free_group_redirect(res, num_redirect + 1)
		&& free_group_redirect(redirect, num_redirect))
		return (NULL);
...
int	add_red(t_list **lexer, t_group *group)
{
	if (((t_token *)((*lexer)->content))->type == e_redirect || \
	((t_token *)((*lexer)->content))->type == e_double_redirect)
	{
		group->redirect = add_redirect(group->redirect, \
		(group->number_redirect)++, \
		((t_token *)((*lexer)->next->content)), \
		((t_token *)((*lexer)->content))->type);
		if (group->redirect == NULL && free_group(group))
			return (MALLOC_ERROR);
	}
	else
	{
		group->reverse_redirect = add_redirect(group->reverse_redirect, \
		(group->number_reverse_redirect)++, \
		((t_token *)((*lexer)->next->content)), \
		((t_token *)((*lexer)->content))->type);
		if (group->reverse_redirect == NULL && free_group(group))
			return (MALLOC_ERROR);
	}
	return (SUCCES);
}
